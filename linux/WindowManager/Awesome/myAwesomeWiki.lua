...
-- awesome.startup_errors string that contains error message
...
-- Tags are essentially our workspaces. 
-- There are a ton of layouts available in awesome. 

-- awesome variables
awful.util.terminal = terminal

awful.tag({ "1", "2", "3", "4", "5", "6", "7", "8", "9" }, s, awful.layout.layouts[10])
-- awful.layout.layouts[10] --> 10 indicates the 10th layout from the layouts table
...
-- Function Add New Tag 
--
local function add_tag(iconOrText)
    awful.tag.add(iconOrText, {
...
-- Move focused client to a new tag
...
-- awful.tag.new (names, screen, layout)	Create a set of tags and attach it to a screen.
-- awful.tag.find_fallback (screen, invalids)	Find a suitable fallback tag.
...
-- awful.tag.viewidx (i[, screen])	View a tag by its taglist index.
...
-- You can set wallpaper with awesome. This is optional, of course.
...
local soundDir = "/opt/sounds/" -- The directory that has the sound files

local startupSound  = soundDir .. "startup.mp3"
local shutdownSound = soundDir .. "shutdown.mp3"
local dmenuSound    = soundDir .. "menu.mp3"
